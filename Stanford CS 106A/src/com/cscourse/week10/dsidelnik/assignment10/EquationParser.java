...
public class EquationParser {
    ...
    public List<String> negativeValueFinder(List<String> expression) {
        ...
        for (int i = 0; i < expression.size(); i++) {
            ...
            if (operator && isOperator(expression.get(i))) {
                sb.append(expression.get(i)).append(expression.get(++i));
                operator = false;
            } else if (!operator && isOperator(expression.get(i))) {
                sb.append(expression.get(i));
                operator = true;
            } else if (isNumber(expression.get(i)) || expression.get(i).equals(")")) {
                sb.append(expression.get(i));
                operator = false;
    ...
    /**
     * Helper method, used to define whether token is parenthesis
     *
     * @param symbol String type token to analyze
     * @return true if symbol represent left or right bracket, else returns false
     */
    public boolean isParenthesis(String symbol) {
        return "()".contains(symbol);
    }
}
